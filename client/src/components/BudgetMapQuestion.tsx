...
interface Props {
  ...
  onChange: (value: BudgetMapQuestionAnswer[]) => void;
  question: SurveyBudgetMapQuestion;
  setDialogOpen: React.Dispatch<React.SetStateAction<boolean>>;
  mobileDrawerOpen?: boolean;
}

export default function BudgetMapQuestion({
  value,
  onChange,
  question,
  setDialogOpen,
  ...
  const valueRef = useRef<BudgetMapQuestionAnswer[]>(null);
  ...
  async function getSubQuestionAnswers() {
    ...
    return await new Promise<BudgetMapQuestionAnswer['subQuestionAnswers']>(
      (resolve) => {
  ...
  const remainingBudget =
    question.budget -
    value.reduce((total, val) => {
      return (
        total +
        (question.options.find((option) => option.id === val.optionId)?.value ??
          0)
      );
    }, 0);
    ...
    <>
      ...
      >
        ...
        <RadioGroup
          ...
          onChange={(event) => {
            setSelectedOptionId(Number(event.currentTarget.value));
          }}
          ...
          {question.options.map((option) => (
            <FormControlLabel
              ...
              label={`${option.text?.[surveyLanguage] ?? ''} ${
                currencyFormat.format(option.value) ?? ''
              }`}
              control={<Radio />}
              sx={{
                lineHeight: 1.2,
